// src/components/EditExpenseScreen.tsx
...
const EditExpenseScreen: React.FC = () => {
  ...
  const expenseToEdit = expenses.find((expense) => expense.id.toString() === id);

  const [formData, setFormData] = useState({
    date: expenseToEdit?.date || '',
    category: expenseToEdit?.category || '',
    amount: expenseToEdit?.amount || 0,
    message: expenseToEdit?.message || ''
  });

  useEffect(() => {
    if (!expenseToEdit) {
      navigate('/expense-list'); // Redirect if expense is not found
    }
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    updateExpense({ ...formData, id: Number(id), amount: Number(formData.amount) });
    navigate('/expense-list'); // Redirect after updating
  };
